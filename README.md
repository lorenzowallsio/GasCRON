...
This project fetches an upstream RSS feed once per day, keeps the latest 5 items by `pubDate`, replaces each selected item's `title` with its `description`, removes item `author`, rewrites `dc:creator` to the original `title`, overrides the channel title to `Gasconnect RSS`, and publishes the transformed RSS output for Walls.io.
